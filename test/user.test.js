...
test('Valid user_id and no filters return all songs', async (t) => {
  const response = await t.context.got.get('user/integer/song',{ searchParams: { song_name: 'Song A' } });
  t.is(response.statusCode, 200);
  t.true(Array.isArray(response.body));
  t.is(response.body.length, 2);
});

// test('Filter by song_name', async (t) => {
//   const response = await t.context.got.get('user/song', 
//     { searchParams: { user_id: 1, song_name: 'Song A' } });
//   t.is(response.statusCode, 200);
//   t.is(response.body.length, 1);
//   t.is(response.body[0].title, 'Song A');
// });

// test('Invalid user_id returns error', async (t) => {
//   const error = await t.throwsAsync(() => t.context.got.get('user/vhfdm/song'));
//   t.is(error.response.statusCode, 400);
//   t.is(error.response.body.message, 'request.params.user_id should be integer');
// });

// test('Invalid filter type returns error', async (t) => {
//   const error = await t.throwsAsync(() => t.context.got.get('user/1/song', { searchParams: { song_name: 123 } }));
//   t.is(error.response.statusCode, 400);
//   t.is(error.response.body.message, 'request.query.song_name should be string');
// });